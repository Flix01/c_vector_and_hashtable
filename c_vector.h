...
/* GLOBAL DEFINITIONS: The following definitions (when used) must be set
   ...
   CV_USE_VOID_PTRS_IN_CMP_FCT          // item_cmp syntax is good for qsort(...)
   CV_MALLOC
   CV_REALLOC
   CV_FREE
   CV_ASSERT
   ...
   CV_NO_STDIO
   CV_NO_STDLIB
   CV_API                               // used for a very small number of common functions
   CV_API_DEC                           // used for function declarations
   CV_API_DEF                           // used for function definitions
*/
...
#include <stdarg.h> /* just for cv_printf(const char *format, ...) */
...
/* text printing and formatting helpers */
CV_API int cv_printf(const char *format, ...)  {
    int done = 1;
#   ifndef CV_NO_STDIO
    va_list arg;
    va_start (arg, format);
...
CV_API void cv_display_bytes(size_t bytes_in)   {
    ...
    for (i=0;i<5;i++)   {
        if (pTGMKB[i]!=0) {
            if (cnt>0) cv_printf(" ");
            cv_printf("%lu %s",pTGMKB[i],names[i]);
            ++cnt;
        }
    }
}
/* private function */
CV_API void cvp_memset(void* dest,int ch,size_t count) {
    /* private function */
#   ifdef CV_DISABLE_CLEARING_ITEM_MEMORY
    {(dest);(ch);(count);}
#   else
...
#   define CV_CMP_FCT_TYPE(CV_TYPE) void
#else
...
struct CV_VECTOR_TYPE(CV_TYPE) {												\
    ...
    int (*const item_cmp)(const CV_CMP_FCT_TYPE(CV_TYPE)*,const CV_CMP_FCT_TYPE(CV_TYPE)*);		/* optional (can be NULL) (for sorted vectors only) */		\
                                                                    \
...
CV_API_DEC void CV_VECTOR_TYPE_FCT(CV_TYPE,_create_with)(CV_VECTOR_TYPE(CV_TYPE)* v,int (*item_cmp)(const CV_CMP_FCT_TYPE(CV_TYPE)*,const CV_CMP_FCT_TYPE(CV_TYPE)*),void (*item_ctr)(CV_TYPE*),void (*item_dtr)(CV_TYPE*),void (*item_cpy)(CV_TYPE*,const CV_TYPE*));      \
CV_API_DEC void CV_VECTOR_TYPE_FCT(CV_TYPE,_create)(CV_VECTOR_TYPE(CV_TYPE)* v,int (*item_cmp)(const CV_CMP_FCT_TYPE(CV_TYPE)*,const CV_CMP_FCT_TYPE(CV_TYPE)*));       \
																	\
CV_EXTERN_C_END														\
...
CV_EXTERN_C_START   												\
																	\
																	\
CV_API void CV_TYPE_FCT(CV_TYPE,_default_item_cpy)(CV_TYPE* a,const CV_TYPE* b) {memcpy(a,b,sizeof(CV_TYPE));}	/* warning: internal function. Implementation can't be modified. */		\
...
CV_API_DEF void CV_VECTOR_TYPE_FCT(CV_TYPE,_swap)(CV_VECTOR_TYPE(CV_TYPE)* a,CV_VECTOR_TYPE(CV_TYPE)* b)  {		\
    ...
    CV_ASSERT(a && b);																\
    memcpy(t,a,sizeof(CV_VECTOR_TYPE(CV_TYPE)));												\
    memcpy(a,b,sizeof(CV_VECTOR_TYPE(CV_TYPE)));												\
    memcpy(b,t,sizeof(CV_VECTOR_TYPE(CV_TYPE)));												\
}																					\
...
CV_API_DEF void CV_VECTOR_TYPE_FCT(CV_TYPE,_resize)(CV_VECTOR_TYPE(CV_TYPE)* v,size_t size)	{			\
    ...
    if (size<v->size)   {if (v->item_dtr) {size_t i;for (i=size;i<v->size;i++) v->item_dtr(&v->v[i]);}}		\
        ...
        if (v->item_ctr || v->item_cpy) cvp_memset(&v->v[v->size],0,(size-v->size)*sizeof(CV_TYPE));		\
...
CV_API_DEF void CV_VECTOR_TYPE_FCT(CV_TYPE,_cpy)(CV_VECTOR_TYPE(CV_TYPE)* a,const CV_VECTOR_TYPE(CV_TYPE)* b) {		\
    ...
    typedef int (*item_cmp_type)(const CV_CMP_FCT_TYPE(CV_TYPE)*,const CV_CMP_FCT_TYPE(CV_TYPE)*);				\
    CV_ASSERT(a && b);																	\
    /* bad init asserts */																\
...
CV_API_DEF void CV_VECTOR_TYPE_FCT(CV_TYPE,_shrink_to_fit)(CV_VECTOR_TYPE(CV_TYPE)* v)	{					\
    if (v)	{																			\
        CV_VECTOR_TYPE(CV_TYPE) o=CV_DEFAULT_STRUCT_INIT;																			\
...
CV_API_DEF void CV_VECTOR_TYPE_FCT(CV_TYPE,_dbg_check)(const CV_VECTOR_TYPE(CV_TYPE)* v)  {					\
        ...
        for (j=0;j<v->size;j++)  {														\
            ...
            if (last_item) {															\
                if (v->item_cmp(last_item,item)>=0) {									\
                    ...
                    ++num_sorting_errors;												\
                    /*cv_printf("[%s] Error: in v[%lu]: item_cmp(%lu,%lu)<=0 [num_items=%lu]\n",CV_XSTR(CV_VECTOR_TYPE_FCT(CV_TYPE,_dbg_check)),j-1,j,v->size);*/	\
                }																		\
            }																			\
            last_item=item;																\
        }																				\
    ...
    cv_printf("\tsize: %lu. capacity: %lu. sizeof(%s): ",v->size,v->capacity,CV_XSTR(CV_TYPE));cv_display_bytes(sizeof(CV_TYPE));cv_printf(".\n");	\
    if (v->item_cmp && v->size) {														\
        if (num_sorting_errors==0) cv_printf("\tsorting: OK.\n");							\
        else cv_printf("\tsorting: NO (%lu sorting errors detected).\n",num_sorting_errors);	\
    }																					\
    cv_printf("\tmemory_used: ");cv_display_bytes(mem_used);								\
    cv_printf(". memory_minimal_possible: ");cv_display_bytes(mem_minimal);				\
    cv_printf(". mem_used_percentage: %1.2f%% (100%% is the best possible result).\n",mem_used_percentage);	\
    /*CV_ASSERT(num_sorting_errors==0);*/ /* When this happens, it can be a wrong user 'itemKey_cmp' function (that cannot sort keys in a consistent way) */	\
                                                                                        ...
                                                                                        \
/* create methods */																	\
CV_API_DEF void CV_VECTOR_TYPE_FCT(CV_TYPE,_create_with)(CV_VECTOR_TYPE(CV_TYPE)* v,int (*item_cmp)(const CV_CMP_FCT_TYPE(CV_TYPE)*,const CV_CMP_FCT_TYPE(CV_TYPE)*),void (*item_ctr)(CV_TYPE*),void (*item_dtr)(CV_TYPE*),void (*item_cpy)(CV_TYPE*,const CV_TYPE*))	{		\
    typedef void (*item_ctr_dtr_type)(CV_TYPE*);										\
    typedef void (*item_cpy_type)(CV_TYPE*,const CV_TYPE*);								\
    typedef int (*item_cmp_type)(const CV_CMP_FCT_TYPE(CV_TYPE)*,const CV_CMP_FCT_TYPE(CV_TYPE)*);				\
                                                                                        \
    ...
    typedef void (* dbg_check_mf)(const CV_VECTOR_TYPE(CV_TYPE)*);								\
                                                                                        \
    CV_ASSERT(v);																		\
    memset(v,0,sizeof(CV_VECTOR_TYPE(CV_TYPE)));													\
...
CV_API_DEF void CV_VECTOR_TYPE_FCT(CV_TYPE,_create)(CV_VECTOR_TYPE(CV_TYPE)* v,int (*item_cmp)(const CV_CMP_FCT_TYPE(CV_TYPE)*,const CV_CMP_FCT_TYPE(CV_TYPE)*))  {CV_VECTOR_TYPE_FCT(CV_TYPE,_create_with)(v,item_cmp,NULL,NULL,NULL);}	\
                                                                                        \
CV_EXTERN_C_END														\
