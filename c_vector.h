...
/* TODO:
	-> add a global definition to allow 'fake member calls' (if it's possible).
		syntax like:
		cv_mystruct v;	// no init
		cv_mystruct_create(&v,...);	// this must init fake members calls too

		v.reserve(&v,10);	// fake member calls ('v' appears twice)
		v.push_back(&v,...);
...
/* Optional and a bit dangerous (C initializer list should be used wherever possible) */
CV_API void CV_VECTOR_TYPE_FCT(_create)(CV_VECTOR_TYPE* v,void (*item_ctr)(CV_TYPE*),void (*item_dtr)(CV_TYPE*),void (*item_cpy)(CV_TYPE*,const CV_TYPE*),int (*item_cmp)(const CV_CMP_TYPE*,const CV_CMP_TYPE*))	{
...
CV_API size_t CV_VECTOR_TYPE_FCT(_linear_search)(CV_VECTOR_TYPE* v,const CV_TYPE* item_to_search,int* match)  {
    int cmp=0;size_t i;
...
CV_API size_t CV_VECTOR_TYPE_FCT(_linear_search_by_val)(CV_VECTOR_TYPE* v,const CV_TYPE item_to_search,int* match)  {return CV_VECTOR_TYPE_FCT(_linear_search)(v,&item_to_search,match);}
CV_API size_t CV_VECTOR_TYPE_FCT(_binary_search)(CV_VECTOR_TYPE* v,const CV_TYPE* item_to_search,int* match)  {
    size_t first=0, last;
    size_t mid;int cmp;
    CV_ASSERT(v && v->item_cmp);
    if (match) *match=0;
...
CV_API size_t CV_VECTOR_TYPE_FCT(_binary_search_by_val)(CV_VECTOR_TYPE* v,const CV_TYPE item_to_search,int* match)    {return CV_VECTOR_TYPE_FCT(_binary_search)(v,&item_to_search,match);}
